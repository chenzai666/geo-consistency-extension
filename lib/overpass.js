...
export function safeJitter(lat, lon, radiusMeters, rng = Math.random) {
  ...
  // coincide with the raw IP centroid, regardless of the RNG output. Clamp
  // radiusMeters up to MIN_DISTANCE_M first so a caller-supplied radius
  // smaller than the minimum can't turn the sqrt-scaling term negative.
  const MIN_DISTANCE_M = 50;
  const effectiveRadiusMeters = Math.max(radiusMeters, MIN_DISTANCE_M);
  const distance = MIN_DISTANCE_M + (effectiveRadiusMeters - MIN_DISTANCE_M) * Math.sqrt(rng());
...
export async function resolveResidentialCoordinate(opts) {
  ...
  try {
    // Cap matches the largest accuracy preset (ACCURACY_PRESETS.city = 3000m
    // in lib/storage-schema.js) so the Overpass search radius never falls
    // short of what the jitter fallback would use for the same setting.
    const query = buildOverpassQuery(lat, lon, Math.min(radiusMeters, 3000));
